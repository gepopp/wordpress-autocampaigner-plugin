  ...
  <div class="ac-flex ac-space-x-5 ac-pb-5 ac-mb-5 ac-border-b ac-border-plugin">
    <div>
      <div class="ac-flex ac-space-x-5 ac-w-full">
        <div class="ac-bg-plugin ac-rounded-full ac-p-2 ac-h-12 ac-w-12 ac-text-white ac-flex-none ac-grow-0">
        ...
        <div class="ac-flex ac-flex-col ac-justify-center">
          <h3 class="ac-text-xl ac-font-semibold">Entwurf am E-Mailserver erstellen</h3>
          <p v-if="waiting" class="ac-animate-pulse">Warte auf Template</p>
          <p v-if="isLoading" class="ac-animate-pulse">Lade Inhalt auf E-Mail Server</p>
  ...
  methods: {
    createDraft() {
      ...
      Axios.post(xhr.ajaxurl, Qs.stringify({
        action: "autocampaigner_create_draft_on_cm",
        nonce: xhr.nonce,
        draft: this.campaign.id
      }))
          .then((rsp) => this.updated = rsp.data)
          .catch((rsp) => this.error = rsp.response.data)
          .then(() => { this.isLoading = false; this.$emit('draftCreated', {cm_id: this.updated}) });
    }
  }
}
</script>
